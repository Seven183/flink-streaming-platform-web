...
//        }
    ...
    public JobRunParamDTO writeSqlToFile(JobConfigDTO jobConfigDTO) {
        ...
        String sqlPath = FileUtils.getSqlHome(systemConfigMap.get(SysConfigEnum.FLINK_STREAMING_PLATFORM_WEB_HOME.getKey())) + fileName;
        ...
        threadPoolExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    localLog.append("HADOOP_CLASSPATH=").append(hadoopClassPath).append(SystemConstant.LINE_FEED);
                    ...
                    switch (jobConfigDTO.getDeployModeEnum()) {
                        ...
                        case LOCAL:
                        case STANDALONE:
                            String address = systemConfigService.getFlinkAddress(jobConfigDTO.getDeployModeEnum());
                            log.info("flink 远程提交地址是 address={}", address);
                            localLog.append("address=").append(address).append(SystemConstant.LINE_FEED);
                            //1、构建执行命令
                            command = CommandUtil.buildRunCommandForCluster(jobRunParamDTO, jobConfigDTO, savepointPath, address);
                            //2、提交任务
            ...
            private void downJar(JobRunParamDTO jobRunParamDTO, JobConfigDTO jobConfigDTO) {
                ...
                int code1 = jobConfigDTO.getJobTypeEnum().getCode();
                if (code == code1) {
                    ...
                    if (MatcherUtils.isHttpsOrHttp(jobConfigDTO.getCustomJarUrl())) {
                        ...
                        String uploadJarsPath = systemConfigService.getUploadJarsPath();
                        String customJarUrl = jobConfigDTO.getCustomJarUrl();
                        pathName = uploadJarsPath + customJarUrl;
                    }
            ...
            private String submitJobForStandalone(String command, JobConfigDTO jobConfig,
                                                  StringBuilder localLog) throws Exception {
    ...
    private void checkSysConfig(Map<String, String> systemConfigMap, DeployModeEnum deployModeEnum) {
        ...
        if (DeployModeEnum.YARN_PER.equals(deployModeEnum) || DeployModeEnum.YARN_APPLICATION.equals(deployModeEnum)) {
